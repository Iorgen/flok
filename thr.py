...
class WorkerThreadPool():
    ...
    def __Tany(self, func, results, *args):
        # TODO refactor as writing into database or on disk in some sort if files
        # And try catchers maybe if not enough inside some sort
        if self.__append:
            results.append(func(*args))
        else:
            results.extend(func(*args))
        self.__logger.info("added to the results")
        return True
